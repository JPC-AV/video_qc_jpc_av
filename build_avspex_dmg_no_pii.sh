...
find dist/dmg/AV-Spex.app/Contents/Frameworks -type f -name "*.so" -o -name "*.dylib" | while read file; do
  codesign --force --timestamp --options runtime --sign "Developer ID Application: Eddy Colloton (4A8****4VX)" "$file"
done
...
find dist/dmg/AV-Spex.app/Contents/MacOS -type f -perm +111 | while read file; do
  codesign --force --timestamp --options runtime --entitlements entitlements.plist --sign "Developer ID Application: Eddy Colloton (4A8****4VX)" "$file"
done

# Sign the main app bundle with entitlements
echo "Signing main application bundle..."
codesign --force --deep --timestamp --options runtime --entitlements entitlements.plist --sign "Developer ID Application: Eddy Colloton (4A8****4VX)" dist/dmg/AV-Spex.app
...
# Sign the DMG itself
codesign --force --timestamp --sign "Developer ID Application: Eddy Colloton (4A8****4VX)" "dist/AV-Spex-$VERSION.dmg"
