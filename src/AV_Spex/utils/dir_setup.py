...
def find_video_file(source_directory, extension=None):
    """Locate the single input video file with the configured container extension.

    Explicit-selection model: only files matching `extension` (default read from
    ChecksConfig.video_file_extension) are considered. QCTools sidecars are excluded.
    Returns None (with a critical log) if zero or more than one match is found.
    """
    if extension is None:
        config_mgr.refresh_configs()
        extension = config_mgr.get_config('checks', ChecksConfig, use_last_used=True).video_file_extension
    ...
    # check if found_videos is more than one
    if found_videos:
        if len(found_videos) == 1:
            ...
            logger.critical(f'More than 1 .{ext} file found in {source_directory}: {found_videos}\n')
            return None
    else:
        logger.critical(f"Error: No .{ext} video file found in the directory: {source_directory}\n")
        return None
...
def find_mkv(source_directory):
    return find_video_file(source_directory, extension='mkv')
...
def initialize_directory(source_directory):
    ...
    video_path = find_video_file(source_directory)
