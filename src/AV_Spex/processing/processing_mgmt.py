...
from AV_Spex.checks.scene_detect import run_scene_detection
...
class ProcessingManager:
    ...
    def process_video_outputs(self, video_path, source_directory, destination_directory, video_id, metadata_differences):
        ...
        # Scene-cut detection results (PySceneDetect). Stage 1: surfaced for
        # the report only — Stage 2 will pass these into frame_analysis and
        # qct-parse audio to annotate cut-coincident false positives.
        processing_results['scene_boundaries'] = qctools_results.get('scene_boundaries', []) if qctools_results else []
...
def process_qctools_output(video_path, source_directory, destination_directory, video_id, report_directory=None, check_cancelled=None, signals=None):
    ...
    # Scene-cut detection (PySceneDetect). Stage 1: standalone — produces a
    # boundaries CSV in the report directory. Stage 2 will hand the list
    # downstream to frame_analysis and qct-parse audio so cut-coincident
    # findings can be annotated as likely false positives.
    ...
    if scene_cfg and getattr(scene_cfg, 'run_tool', False):
        ...
        if not report_directory:
            report_directory = dir_setup.make_report_dir(source_directory, video_id)
        boundaries = run_scene_detection(
            video_path=video_path,
            report_directory=report_directory,
            video_id=video_id,
            detector=getattr(scene_cfg, 'detector', 'content'),
            threshold=getattr(scene_cfg, 'threshold', 27.0),
            min_scene_len=getattr(scene_cfg, 'min_scene_len', 15),
            check_cancelled=check_cancelled,
            signals=signals,
        )
        results['scene_boundaries'] = boundaries or []
        if signals and hasattr(signals, 'step_completed'):
            signals.step_completed.emit("Scene Detection")
        logger.info("")  # blank line for readability
